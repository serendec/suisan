...
<tbody>
	...
	<tr>
		...
		<td>
			<?php if( $repeater['水準'] === '低位'){
				$label = 'pink';
			} elseif( $repeater['水準'] === '高位' ) {
				$label = 'grn';
			} else {
				$label = 'org';
			}
			if( $repeater['動向'] === '減少'){
				$arrow = 'arw-down';
			} elseif( $repeater['動向'] === '増加' ) {
				$arrow = 'arw-up';
			} else {
				$arrow = '';
			}?>
			<!-- 動向 -->
			<div class="c-label c-label--sts c-label--<?php echo $label; ?> <?php echo $arrow; ?>"><i class="far fa-long-arrow-right"></i></div>
		</td>
		<td>
			<!-- 水準 -->
			<div class="c-label c-label--sts c-label--<?php echo $label; ?>"><?php echo $repeater['水準']; ?></div>
		</td>
	</tr>
	<?php
